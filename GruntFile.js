    ...
    grunt.initConfig({
        ...
        concat: {
            css: {
                options: {
                    separator: '\n',
                    banner: '/* nvd3 extra version ' + _pkg.version + ' (' + _pkg.url + ') ' +
                        '<%= grunt.template.today("yyyy-mm-dd") %> */\n'
                },
                src: [
                    'src/css/*.css'
                ],
                dest: 'build/nv.d3.extra.css'
            },
            js: {
                options: {
                    separator: '',
                    banner: '/* nvd3 extra version ' + _pkg.version + ' (' + _pkg.url + ') ' +
                        '<%= grunt.template.today("yyyy-mm-dd") %> */\n' + '(function(){\n',
                    footer: '\nnv.extraVersion = "' + _pkg.version + '";\n})();',
                    sourceMap: true,
                    sourceMapName: 'build/nv.d3.extra.js.map',
        ...
        uglify: {
            options: {
                ...
                sourceMapIn : 'build/nv.d3.extra.js.map',
                banner: '/* nvd3 extra version ' + _pkg.version + ' (' + _pkg.url + ') ' +
                    '<%= grunt.template.today("yyyy-mm-dd") %> */\n'
            },
            js: {
                files: {
                    'build/nv.d3.extra.min.js': ['build/nv.d3.extra.js']
                }
            }
        },
        replace: {
        ...
        copy: {
            css: {
                files: [
                    { src: 'src/nv.d3.extra.css', dest: 'build/nv.d3.css' }
                ]
            }
        },
        postcss: {
        ...
        cssmin: {
            ...
            dist: {
                files: {
                    'build/nv.d3.extra.min.css' : ['build/nv.d3.extra.css']
                }
            }
        },
        // karma: {
